...
def rotation_y_matrix(theta, rank=3):
    assert rank in (3, 4)
...
def compute_3d_iou(bbox_3d_1, bbox_3d_2, handle_visibility, class_name_1, class_name_2):
    ...
    if bbox_3d_1 is None or bbox_3d_2 is None: return -1
    ...
    if symmetry_flag:
        n = 20; theta = 2*math.pi/n

        rotation_matrix = rotation_y_matrix(theta)
        max_iou = 0
        for i in range(n):
            bbox_center = np.mean(bbox_3d_1, -1, keepdims=True)
            bbox_3d_1 = rotation_matrix @ (bbox_3d_1 - bbox_center) + bbox_center
...
def compute_3d_iou_new(RT_1, RT_2, scales_1, scales_2, handle_visibility, class_name_1, class_name_2):
    ...
    if RT_1 is None or RT_2 is None: return -1

    symmetry_flag = False
    if (class_name_1 in ['bottle', 'bowl', 'can'] and class_name_1 == class_name_2) \
        or (class_name_1 == 'mug' and class_name_1 == class_name_2 and handle_visibility==0):
        noc_cube_1 = get_3d_bbox(scales_1, 0)
        noc_cube_2 = get_3d_bbox(scales_2, 0)
        bbox_3d_2 = transform_coordinates_3d(noc_cube_2, RT_2)
        ...
        n = 20; max_iou = 0
        for i in range(n):
            rotated_RT_1 = RT_1@rotation_y_matrix(2*math.pi*i/float(n), 4)
            max_iou = max(max_iou, asymmetric_3d_iou(rotated_RT_1, RT_2, scales_1, scales_2))
    else: max_iou = asymmetric_3d_iou(RT_1, RT_2, scales_1, scales_2)
    return max_iou
...
#         print(np.linalg.det(R1), np.linalg.det(R2))
    ...
    ## symmetric when rotating around y-axis
    elif synset_names[class_id] == 'mug' and (handle_visibility==0).any():
        y = np.array([0, 1, 0])
        y1 = R1 @ y
        y2 = R2 @ y
        theta = np.arccos(y1.dot(y2) / (np.linalg.norm(y1) * np.linalg.norm(y2)))
...
def compute_RT_projection_2d_symmetry(RT_1, RT_2, class_id, handle_visibility,
    mesh_vertices, intrinsics, synset_names, num_rotation=20):
    '''
    :param RT_1: [4, 4]. homogeneous affine transformation
    :param RT_2: [4, 4]. homogeneous affine transformation
    :param vertices: [3, N].
    ...
    if (synset_names[class_id] in ['bottle', 'can', 'bowl']) or \
        ...
        for i in range(1, num_rotation):
            ...
            coords_3d_2 = transform_coordinates_3d(rotation_y_matrix(theta)@vertices, RT_2)
            projected_2 = calculate_2d_projections(coords_3d_2, intrinsics)
...
def mold_mask(mask, config): pass
...
def compute_3d_matches(gt_class_ids, gt_RTs, gt_scales, gt_handle_visibility, synset_names, pred_boxes,
        pred_class_ids, pred_scores, pred_RTs, pred_scales, iou_3d_thresholds, score_threshold=0):
    """Finds matches between prediction and ground truth instances.
    Returns:
...
def compute_RT_overlaps(gt_class_ids, gt_RTs, gt_handle_visibility,
                        pred_class_ids, pred_RTs, synset_names):
    ...
    for i in range(num_pred):
        for j in range(num_gt):
            overlaps[i, j, :] = compute_RT_degree_cm_symmetry(pred_RTs[i], 
                gt_RTs[j], gt_class_ids[j], gt_handle_visibility[j], synset_names)
    return overlaps
    ...
    for i in range(num_pred):
        for j in range(num_gt):
            overlaps[i, j] = compute_RT_projection_2d_symmetry(pred_RTs[i], 
                gt_RTs[j], gt_class_ids[j], gt_handle_visibility[j],
                meshes[gt_class_ids[j]], intrinsics, synset_names)
    return overlaps
        ...
        for i in range(num_pred):
            ...
            for j in sorted_ixs:
                ...
                if gt_matches[p, j] > -1 or pred_class_ids[i] != gt_class_ids[j]:
                    continue
                # If we reach IoU smaller than the threshold, end the loop
                if overlaps[i, j] > projection_thres: continue
                gt_matches[p, j] = i
                pred_matches[p, i] = j
                break
    return gt_matches, pred_matches
...
def compute_degree_cm_mAP(final_results, synset_names, log_dir, degree_thresholds=[360],
    ...
    for progress, result in enumerate(final_results):
        ...
        if len(gt_class_ids) == 0 and len(pred_class_ids) == 0: continue
        ...
        for cls_id in range(1, num_classes):
            ...
            iou_cls_gt_match, iou_cls_pred_match, _, iou_pred_indices = compute_3d_matches(
                cls_gt_class_ids, cls_gt_RTs, cls_gt_scales, cls_gt_handle_visibility, synset_names,
                cls_pred_bboxes, cls_pred_class_ids, cls_pred_scores, cls_pred_RTs, cls_pred_scales, iou_thres_list)
            ...
            RT_overlaps = compute_RT_overlaps(cls_gt_class_ids, cls_gt_RTs, cls_gt_handle_visibility, 
                                              cls_pred_class_ids, cls_pred_RTs, synset_names)


            pose_cls_gt_match, pose_cls_pred_match = compute_match_from_degree_cm(RT_overlaps, 
                cls_pred_class_ids, cls_gt_class_ids, degree_thres_list, shift_thres_list)
    ...
    for cls_id in range(1, num_classes):
        ...
        for s, iou_thres in enumerate(iou_thres_list):
            iou_3d_aps[cls_id, s] = compute_ap_from_matches_scores(iou_pred_matches_all[cls_id][s, :],
                iou_pred_scores_all[cls_id][s, :], iou_gt_matches_all[cls_id][s, :])    
    ...
    if use_matches_for_pose: prefix='Pose_Only_'
    else: prefix='Pose_Detection_'

    pose_dict_pkl_path = os.path.join(log_dir, prefix+'AP_{}-{}degree_{}-{}cm.pkl'.format(
        ...
        for j, shift_thres in enumerate(shift_thres_list):
            ...
            for cls_id in range(1, num_classes):
                ...
                pose_aps[cls_id, i, j] = compute_ap_from_matches_scores(cls_pose_pred_matches_all, 
                    cls_pose_pred_scores_all, cls_pose_gt_matches_all)
    ...
    for cls_id in range(1, num_classes):
        ...
        output_path = os.path.join(log_dir, prefix+'AP_{}_{}-{}degree_{}-{}cm.png'.format(
    ...
    output_path = os.path.join(log_dir, prefix+'mAP_{}-{}degree_{}-{}cm.png'.format(
        degree_thres_list[0], degree_thres_list[-2], shift_thres_list[0], shift_thres_list[-2]))
    plt.colorbar()
    plt.savefig(output_path)
    plt.close(fig_pose)
    ...
    for progress, result in enumerate(final_results):
        ...
        for cls_id in range(1, num_classes):
            ...
            RT_projection_2d_overlaps = compute_RT_projection_2d_overlaps(
                cls_gt_class_ids, cls_gt_RTs, cls_gt_handle_visibility, cls_pred_class_ids,
                cls_pred_RTs, meshes, intrinsics, synset_names)
    ...
    for i, projection_thres in enumerate(projection_thres_list):                
        for cls_id in range(1, num_classes):
            projection_2d_aps[cls_id, i] = compute_ap_from_matches_scores(projection_pred_matches_all[cls_id][i, :],
                projection_pred_scores_all[cls_id][i, :], projection_gt_matches_all[cls_id][i, :])
    ...
    for progress, result in enumerate(final_results):
        ...
        for cls_id in range(1, num_classes):
            ...
            gt_match, pred_match, overlaps, pred_indices = compute_matches(cls_gt_bboxes, cls_gt_class_ids, 
                cls_gt_masks, cls_pred_bboxes, cls_pred_class_ids, cls_pred_scores, cls_pred_masks, 0.5)
                ...
                for i in range(len(pred_match)):
                    if pred_match[i] > -1:
                        ...
                        mean_coord_dist = compute_mean_l1_coord_diff(cls_pred_masks_sorted[..., i], 
                            cls_gt_masks[..., j], cls_pred_coords_sorted[..., i, :], 
                            cls_gt_coords[..., j, :], synset_names, cls_id)
...
def compute_3d_matches_degree_cm(gt_class_ids, gt_RTs, gt_scales, pred_boxes, pred_class_ids,
        pred_scores, pred_RTs, synset_names, thres_shift = 5, thres_degree=5, bound_threshold=5):
    """Finds matches between prediction and ground truth instances.
    Returns:
        gt_match: 1-D array. For each GT box it has the index of the matched
                  predicted box.
    ...
    for i in range(num_pred):
        for j in range(num_gt):
            overlaps[i, j] = compute_RT_upper_bound_symmetry(pred_RTs[i], gt_RTs[j], 
                gt_class_ids[j], synset_names, thres_shift=thres_shift, thres_theta=thres_degree)
...
def load_mesh(mesh_path, is_save=False, is_normalized=False, is_flipped=False):
    with open(mesh_path, 'r') as f: lines = f.readlines()
...
def draw(img, imgpts, axes, color, lw=2):
    imgpts = np.int32(imgpts).reshape(-1, 2)
    # draw ground layer in darker color
    color_ground = (int(color[0] * 0.3), int(color[1] * 0.3), int(color[2] * 0.3))
    for i, j in zip([4, 5, 6, 7],[5, 7, 4, 6]):
        img = cv2.line(img, tuple(imgpts[i]), tuple(imgpts[j]), color_ground, lw)

    # draw pillars in blue color
    color_pillar = (int(color[0]*0.6), int(color[1]*0.6), int(color[2]*0.6))
    for i, j in zip(range(4),range(4,8)):
        img = cv2.line(img, tuple(imgpts[i]), tuple(imgpts[j]), color_pillar, lw)
    
    # finally, draw top layer in color
    for i, j in zip([0, 1, 2, 3],[1, 3, 0, 2]):
        img = cv2.line(img, tuple(imgpts[i]), tuple(imgpts[j]), color, lw)

    # draw axes
    img = cv2.line(img, tuple(axes[0]), tuple(axes[1]), (0, 0, 255), lw)
    img = cv2.line(img, tuple(axes[0]), tuple(axes[3]), (255, 0, 0), lw)
    img = cv2.line(img, tuple(axes[0]), tuple(axes[2]), (0, 255, 0), lw) ## y last
    return img
...
def draw_text(draw_image, bbox, text, draw_box=False):
    text_pos = bbox[1], bbox[0]-5
    #ret, baseline = cv2.getTextSize(text, 4, 0.6, 1)    
    if draw_box: # text_pose is the bottom-left corner of the text
        cv2.rectangle(draw_image, (bbox[1], bbox[0]), (bbox[3], bbox[2]), (255, 0, 0), 2)
    cv2.putText(draw_image, text, text_pos, 4, 0.6, (255,255,255), 1)
    return draw_image
...
def draw_detections(image, save_dir, data_name, image_id, intrinsics, synset_names, draw_rgb_coord,
        ...
        draw_gt=True, draw_pred=True, draw_tag=0.6):
    alpha = float(draw_tag)
    if draw_gt:
    ...
    if draw_pred:
        ...
        if gt_class_ids is not None:
            gt_match, pred_match, _, pred_indices = compute_matches(gt_bbox, gt_class_ids, gt_mask,
                pred_bbox, pred_class_ids, pred_scores, pred_mask, 0.5)
...
def draw_coco_detections(image, save_dir, data_name, image_id, synset_names, draw_rgb_coord, gt_bbox, 
        gt_class_ids, gt_mask, pred_bbox, pred_class_ids, pred_mask, pred_coord, pred_scores):
...
def align(class_ids, masks, coords, depth, intrinsics, synset_names, image_path,
    ...
    for i in range(num_instances):
        ...
        try:
            ...
            print(f'align {class_name}:\t{elapsed}s')
            elapses.append(elapsed)
        except Exception as e:
            msg = f'[Error] Fail aligning {synset_names[class_id]} in {image_path}: {str(e)}.'
            print(msg); error_messages += msg + '\n'
    ...
    print('Total alignment time: {:03f}s'.format(sum(elapses)))
    return RTs, bbox_scales, error_messages, elapses


def align_ICP(class_ids, masks, depth, intrinsics, synset_names, image_path,
    save_path=None, if_norm=False, verbose=False):
    num_instances = len(class_ids)
    error_messages = ''
    ...
    for i in range(num_instances):
        ...
        glob_paths = './data/pts/real_test/{}*.txt'.format(synset_names[class_ids[i]])
        print(glob_paths)
