...
class MaskRCNN():
    ...
    def __init__(self, mode, config, model_dir):
        ...
        if mode=='training': self.set_log_dir()
        ...
        if mode == "training":
            ...
            if config.COORD_USE_BINS:
                ...
                mrcnn_coord_x_bin_reshape = KL.Lambda(lambda t: tf.reshape(t,
                    [-1, mrcnn_coord_x_shape[-1]]))(mrcnn_coord_x_bin)
                ...
                mrcnn_coord_y_bin_reshape = KL.Lambda(lambda t: tf.reshape(t,
                    [-1, mrcnn_coord_y_shape[-1]]))(mrcnn_coord_y_bin)
                ...
                mrcnn_coord_z_bin_reshape = KL.Lambda(lambda t: tf.reshape(t,
                    [-1, mrcnn_coord_z_shape[-1]]))(mrcnn_coord_z_bin)
            ...
            # direct regress
                ...
                if config.COORD_SHARE_WEIGHTS:
                    ...
                    mrcnn_coord_x, mrcnn_coord_y, mrcnn_coord_z, mrcnn_coord_feature = fn(rois,
                        mrcnn_feature_maps, config.IMAGE_SHAPE, config.COORD_POOL_SIZE,
                        config.NUM_CLASSES, config.USE_BN)
    ...
    def load_weights(self, filepath, by_name=False, exclude=None):
        ...
        #self.set_log_dir(filepath) # Update the log directory
