...
def main(weight, phi=0, dst=None):
    ...
    model, image_size = build_model_and_load_weights(phi, num_classes, score_threshold, weight)
    
    #inferencing
    print("\nStarting inference...\n")
    while True:
        #load image
        for j in range(8): cam.grab()
        ret, image = cam.read()
        if not ret: continue
        img = image.copy()
        ...
        draw_detections(img, boxes, scores, labels, rotations, translations,
                        class_to_bbox_3D = class_to_3d_bboxes, camera_matrix = camera_matrix,
                        label_to_name = class_to_name, draw_bbox_2d = draw_bbox_2d, draw_name = draw_name)
        ...
        cv2.imshow('pose', img); k = cv2.waitKey(1)
...
def build_model_and_load_weights(phi, num_classes, score_threshold, weight):
    """
    ...
    _, efficientpose_prediction, _ = build_EfficientPose(phi,
        num_classes = num_classes, num_anchors = 9, freeze_bn = True,
        score_threshold = score_threshold, num_rotation_parameters = 3,
        print_architecture = False)
    
    print("\nDone!\n\nLoading weights...")
    efficientpose_prediction.load_weights(weight, by_name=True)
    print("Done!")
...
if __name__ == '__main__':
    weight = 'weights/Occlusion/phi_3_occlusion_best_ADD(-S).h5'
    main(weight, phi=3)
