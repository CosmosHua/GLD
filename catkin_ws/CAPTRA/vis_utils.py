...
def plot3d_pts(pts, pts_name=None, s=2, dpi=100, title_name=None,
    ...
    for m in range(num):
        ...
        for n in range(len(pts[m])):
            pt = (pts[m][n][:,0], pts[m][n][:,1], pts[m][n][:,2])
            if cur_color is not None:
                ax.scatter(*pt, marker=all_poss[0], s=s, c=cur_color[n], alpha=1.0)
            elif color_channel is None:
                ax.scatter(*pt, marker=all_poss[0], s=s, cmap=colors[n], label=pts_name[m][n], alpha=1.0)
            else:
                if colorbar: rgb_encoded = color_channel[n]
                else:
                    mi = np.amin(color_channel[n], axis=0, keepdims=True)
                    mx = np.amax(color_channel[n], axis=0, keepdims=True)
                    rgb_encoded = (color_channel[n] - mi) / np.array(mx - mi)
                if len(pts[m]) == 3 and n == 2:
                    p = ax.scatter(*pt, marker=all_poss[4], s=s, c=rgb_encoded, label=pts_name[m][n])
                else:
                    p = ax.scatter(*pt, marker=all_poss[0], s=s, c=rgb_encoded, label=pts_name[m][n])
                if colorbar:
                    if color_range is not None: p.set_clim(*color_range)
                    fig.colorbar(p)
        ...
        if use_pts_name: plt.legend(loc='lower left')
    ...
    if save_fig: safe_save_fig(fig, pjoin(save_path, f'{save_title}.png'))
    plt.close()
...
def plot_bbox_with_pts(points_list, bcm, valid=None, view_angle=None, lims=None,
    ...
    if valid is None: valid = np.ones(len(points_list))
    if lims is None: lims = compute_lims_for_pts(points_list)
    ...
    plt.tight_layout()
    if show_fig: plt.show()
    if save_fig: safe_save_fig(fig, save_path)
    plt.close(); return lims


def plot_bboxes_on_image(image, bboxes, valid_mask=None, show_fig=True, save_fig=False, out_path=None):
    '''
    ...
    '''
    num_objs = len(bboxes)
    if valid_mask is None: valid_mask = np.ones(num_objs)
    ...
    ax.imshow(image); ax.add_collection(p)
    for k in range(num_objs):
        ...
        for pts_order in pixels_all:
            line1 = []
            for j in pts_order: line1.append(tuple(img_pts[j]))
            line1_xs, line1_ys = zip(*line1)
            ax.add_line(Line2D(line1_xs, line1_ys, linewidth=1.5, color=color))

    plt.axis('off'); #plt.tight_layout()
    im = plt2cv(fig) # OR: im = plt2cv(plt)
    #safe_save_fig(fig, out_path); im = cv2.imread(out_path); os.remove(out_path)
    if show_fig: cv2.imshow('captra',im); cv2.waitKey(5)
    plt.close(); return im # RGBA


def plt2cv(fig): # using buffer, great way!
    buffer = io.BytesIO() # apply & save to buffer
    fig.savefig(buffer, format='png') # plt.savefig
    buffer.seek(0) # read buffer using PIL or CV2
    #im = np.asarray(bytearray(buffer.read()), np.uint8)
    im = np.frombuffer(buffer.read(), np.uint8)
    im = cv2.imdecode(im, cv2.IMREAD_UNCHANGED) # RGBA
    #im = np.asarray(PIL.Image.open(buffer))[...,2::-1]
    buffer.close(); return im # cv2.COLOR_RGBA2BGR
