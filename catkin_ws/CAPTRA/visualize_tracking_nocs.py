...
def parse_args():
    ...
    parser.add_argument('--show_fig', action='store_true', default=True, help='show figure')
...
def get_posed_bbox(pose, bbox):
    pose = {k:torch.tensor(v).float() for k,v in pose.items()}
...
def plot_sequence(img_path, data_dict, output_path, scene_name, cam_intrinsics, scale=1000.0,
                  show_fig=True, save_fig=True, depth=False):
    frame_nums = []; frame_dict = {}
    for k,v in data_dict.items():
        frame_nums += [i[0] for i in v['frame_nums']]
    frame_nums = sorted(list(np.unique(frame_nums)))
    for k,v in data_dict.items():
        frame_dict[k] = {v['frame_nums'][i][0]:i for i in range(len(v['frame_nums']))}

    suffix = 'depth' if depth else 'color'
    vid = pjoin(output_path, f'{scene_name}.mp4')
    vid = cv2.VideoWriter(vid, fourcc, 25, (640, 480))
    for j, frame_num in tqdm(enumerate(frame_nums)):
        image_path = pjoin(img_path, scene_name, f'{frame_num}_{suffix}.png')
        if depth:
            image = cv2.imread(image_path, -1)
            image = np.stack([image]*3, axis=-1)
        else: image = cv2.imread(image_path)[..., ::-1]

        bbox_list = []
        for k,v in data_dict.items():
            if frame_num not in frame_dict[k]: continue
            i = frame_dict[k][frame_num]
            corners = v['gt']['corners'] if i == 0 else v['pred']['corners'][i]
            ...
            img_bbox[..., 1] = len(image) - img_bbox[..., 1]
            img_bbox = img_bbox.reshape(8, 2)
            bbox_list.append(img_bbox)
        bbox_list = np.stack(bbox_list, axis=0)

        out_path = pjoin(output_path, f'{scene_name}_{frame_num}.png')
        im = plot_bboxes_on_image(image, bbox_list, None, show_fig, save_fig, out_path)
        if save_fig: vid.write(im[...,:3]) # can't write alpha-channel
    vid.release()


def plot_all_sequences(img_path, exp_list, output_path, scene_list, cam_intrinsics, scale=1000.0,
                       show_fig=True, save_fig=False, depth=False):
    ...
    for scene_name in scene_list:
        plot_sequence(img_path, scene_dict[scene_name], output_path, scene_name, cam_intrinsics,
                      scale=scale, show_fig=show_fig, save_fig=save_fig, depth=depth)


if __name__ == "__main__":
    args = parse_args()
    exp_list = [pjoin(args.exp_path, exp_name) for exp_name in exp_list]
    intrinsics = np.array([[591.0125, 0, 322.525], [0, 590.16775, 244.11084], [0, 0, 1]])
    if args.scene is not None: scene_list = [args.scene]
    else: scene_list = [f'scene_{i+1}' for i in range(6)]
    plot_all_sequences(args.img_path, exp_list, args.output_path, scene_list, intrinsics,
        scale=1000.0, show_fig=args.show_fig, save_fig=args.save_fig, depth=args.depth)
