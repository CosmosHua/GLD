...
def generate_bins(bins):
    interval = 2 * np.pi / bins # center of the bin
    return (np.array(range(bins))+0.5) * interval
...
class Dataset(data.Dataset):
    ...
    def format_label(self, line):
        ...
        label = {'Class': Class, 'Box_2D': Box_2D, 'Dimensions': Dimension,
                'Alpha': Alpha, 'Orientation': Orientation, 'Confidence': Confidence}
        return label
    ...
    def parse_label(self, label_path):
        ...
        with open(label_path, 'r') as f:
            for line in f:
                ...
                buf.append({'Class': Class, 'Box_2D': Box_2D, 'Dimensions': Dimension,
                        'Location': Location, 'Alpha': Alpha, 'Ry': Ry})
        return buf
