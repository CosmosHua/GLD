...
def show_feature(src, sh=0):
    for i in os.listdir(f'{src}/images'):
        im, pt = SfM_feat_uv(i, src)
        for p in pt: im = CIRCLE(im, p, COLOR())
        cv2.imwrite(f'{src}/{i}_feat.jpg', im)
...
def show_match_gz(src, M, key=0, src2='', sh=5):
    if not os.path.isdir(src2): src2 = src
    dst = f'{src}/mt_gz'; os.makedirs(dst, exist_ok=True)
...
TID = lambda i,j,T: [k for k in T[i]['tid'] if k in T[j]['tid']]
##########################################################################################
...
def show_match_csv(src, T, key=TID, src2='', sh=5):
    if not os.path.isdir(src2): src2 = src
    dst = f'{src}/mt_csv'; os.makedirs(dst, exist_ok=True)
    for (i,j) in combinations(T,2):
        tid = key(i, j, T) # {im: {tid,fid,xys,rgb}}
        if len(tid)<4: continue
        i, j, dir1, dir2 = sort_pair(i, j, src, src2)
        pt1 = np.array(T[i]['xys'])[IDX(T[i]['tid'], tid)]
        pt2 = np.array(T[j]['xys'])[IDX(T[j]['tid'], tid)]
...
def SfM_gcp_gz(GPS, RTK='', thd=0.5, dst=0):
    ...
    for (im1,im2),fid in M.items(): # fid=[fid1,fid2]
        ...
        if dst: # show & save
            #if len(fid): print('\n'.join(gcp[-len(fid):]))
            show_match([im1, im2, GPS, RTK], [xy1, xy2], dst=dst)
...
##########################################################################################
def SfM_img_lla2(GPS, RTK, dst, mix=0):
    sub = f'RTK_{os.path.basename(RTK)}'
    merge_dir(RTK, f'{dst}/{sub}/images'); INFO(SYS)
    RTK = f'{dst}/{sub}'; SfM_config(RTK, SfM_CFG)
    if not os.path.isdir(f'{RTK}/matches'):
        SfM_exif_dop(RTK); SfM_cmd(RTK, (1,2))
    if not os.path.isfile(f'{RTK}/reconstruction.json'):
        SfM_cmd(RTK, (3,4)); #SfM_cmd(RTK, 'export_ply')
    assert os.path.isfile(f'{RTK}/reconstruction.json')

    sub = f'GPS_{os.path.basename(GPS)}{mix}'
    merge_dir(GPS, f'{dst}/{sub}/images')
    GPS = f'{dst}/{sub}'; SfM_config(GPS, SfM_CFG)
    SfM_create_gcp(GPS, RTK, mix) # GPS/gcp_list.txt

    cfg = {'bundle_use_gcp':True, 'bundle_use_gps':False}
    SfM_config(GPS, cfg) # enable GCP, disable GPS
    if not os.path.isdir(f'{GPS}/matches'):
        SfM_cmd(GPS, 'match_features') # match
    if not os.path.isfile(f'{GPS}/reconstruction.json'):
        SfM_cmd(GPS, (3,4)); #SfM_cmd(GPS, 'export_ply')
    export_geocoords_img(GPS); INFO('\nALL DONE!')


##########################################################################################
def ODM_img_lla2(GPS, RTK, dst, mesh=0):
    from odm_align import odm_align; INFO(SYS)
    ...
    if mesh>0: odm_cfg['end-with'] = 'mvs_texturing' # texturing_25d
    if mesh>0: ODM_cmd(GCP, odm_cfg); odm_align(GCP, RTK) # GCP 25d
    if mesh>1: odm_cfg.pop('gcp'); ODM_cmd(RTK, odm_cfg); odm_align(RTK)
    if mesh>2: # odm_texturing_25d of original GPS
        GPS = f'{dst}/odm-GPS-{gps}'; merge_dir(GCP+'/images', GPS+'/images')
        merge_dir(GCP+'/opensfm/features', GPS+'/opensfm/features')
        merge_dir(GCP+'/opensfm/matches', GPS+'/opensfm/matches')
        ODM_cmd(GPS, odm_cfg); odm_align(GPS, RTK) # GPS 25d'''
    INFO('\nALL DONE!')
...
def export_gz_uv(GPS, RTK):
    M = SfM_parse_gz(GPS,'matches_gcp'); res = {}
    for (i,j),fid in M.items():
        _, xy1 = SfM_feat_uv(i, src=GPS, idx=fid[:,0])
        _, xy2 = SfM_feat_uv(j, src=RTK, idx=fid[:,1])
        res[f'{i}-{j}'] = np.hstack([xy1,xy2]).tolist()
    with open(f'{GPS}/gz_uv.json','w') as f:
...
if __name__ == '__main__':
    ...
    #show_match_gz(dst, SfM_parse_gz(dst))
    #show_match_csv(dst, SfM_parse_csv(dst))
    #SfM_exif_dop(dst); SfM_cmd(dst, 'reconstruct')
    #export_geocoords_img('opensfm_gcp',0)
    #export_geocoords_img('opensfm_gcp3',1)
    #SfM_gcp_gz('out2/GPS', 'out2/RTK')
    #export_gz_uv('out2/GPS', 'out2/RTK')
    #SfM_img_lla2('GPS', 'RTK', 'out2')
    #show_gcp_gz('out2/GPS', 'out2/RTK')
    #dedup_gcp('20211108')
    ODM_img_lla2(x.gps, x.rtk, x.dst, x.mesh)
    #parse_log(x.log)
